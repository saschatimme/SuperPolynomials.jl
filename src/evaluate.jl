...
function evaluate_impl(::Type{T}, f::Type{Polynomial{S, NVars, NTerms, Val{Exponents}}}) where {T, S, NVars, NTerms, Exponents}
    M = convert_to_matrix(NVars, NTerms, Exponents)
    evaluate_impl(promote_type(T, S), M)
end

@generated function evaluate(f::Polynomial{S, NVars, NTerms, Val{Exponents}}, x::AbstractVector{T}) where {T, S, NVars, NTerms, Exponents}
    evaluate_impl(T, f)
end
